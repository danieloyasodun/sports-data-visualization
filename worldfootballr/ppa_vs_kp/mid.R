...
passing_data <- read_csv(file.path(data_folder, "big5_player_passing_2025.csv")) %>%
  select(Player, KP, Pos, PPA)  # Just the relevant columns
...
merged_data <- merged_data %>%
  mutate(
    PPA_per_90 = (PPA / Min_Playing.Time) * 90,
...
top_ppa <- filtered_data %>% arrange(desc(PPA_per_90)) %>% head(15)
...
filtered_data <- filtered_data %>%
  mutate(
    Player_Type = case_when(
      ...
      Player %in% top_ppa$Player ~ "Top 15 PPA",
...
# Plot
plot <- ggplot(filtered_data, aes(x = PPA_per_90, y = KP_per_90)) +
  ...
  labs(
    title = "Passes into Penalty Area per 90 vs. Key Passes per 90(24-25 Season Big 5 Leagues)",
    x = "Passes into Penalty Area per 90",
    y = "Key Passes per 90",
    color = "Player Type",
...
# Save plot
ggsave("big5_ppa_vs_kp_top15.png", plot = plot, width = 10, height = 6)